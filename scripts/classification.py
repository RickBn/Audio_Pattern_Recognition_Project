...
knn_test = []
for n in np:
	knn_m, knn_cm, knn_cv_train, knn_cv_test = repeated_k_fold(df, repetitions, num_folds, KNeighborsClassifier(n_neighbors=n), scaler)
...
#Classification
knn_r, knn_cm, knn_cv_train, knn_cv_test = repeated_k_fold(df, repetitions, num_folds, KNeighborsClassifier(n_neighbors=4), scaler)
...
tree_test = []
for n in np:
	tree_m, tree_cv_train, tree_cv_test = repeated_k_fold(df, repetitions, num_folds, DecisionTreeClassifier(criterion='entropy', max_depth=n), scaler)
...
#Classification
tree_r, tree_cm, tree_cv_train, tree_cv_test = repeated_k_fold(df, repetitions, num_folds, DecisionTreeClassifier(criterion='entropy'), scaler)
...
#Classification
mlp_r, mlp_cm, mlp_cv_train, mlp_cv_test = repeated_k_fold(df, repetitions, num_folds, MLPClassifier(hidden_layer_sizes=(128, 64, 32), solver='adam', max_iter=1000), scaler)
...
#Classification
svm_r, svm_cm, svm_cv_train, svm_cv_test = repeated_k_fold(df, repetitions, num_folds, svm.SVC(C=5, decision_function_shape='ovo', kernel='rbf'), scaler)
...
# svm_test = []
# for n in np:
# 	svm_m, svm_cv_train, svm_cv_test = repeated_k_fold(df, repetitions, num_folds, svm.SVC(C=5, decision_function_shape='ovo', kernel='poly', degree=n), scaler)
# 	svm_train.append(svm_cv_train)
# 	svm_test.append(svm_cv_test)
#
# fig, ax = plt.subplots(figsize=(10, 5))
